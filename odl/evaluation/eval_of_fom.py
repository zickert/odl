...
def compare_reco_matrix(fom_list, reco_op_list, data_list, phantom,
    ...
    confidence_vals : numpy arrays
        Contains matrices that can be used to draw conclusions...
    """
    ...
    for i in range(num_reco):
        for j in range(num_reco):
            if i < j:
                ...
                # OLD! REMOVE!
                #compare_reco_confidence_interval(fom_list, reco_op_1, reco_op_2,data_list, phantom, conf_level)
                #print([np.zeros(num_fom), x_min, x_max])
                #print(np.median([np.zeros(num_fom), x_min, x_max], axis = 0))

                #reco_vals[:, j,i]=-reco_vals[:, i,j]
                #reco_vals[j,i]=x_mean
